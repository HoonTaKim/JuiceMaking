...
public class FruitSpwner : MonoBehaviour
{
    ...
    private void Init()
    {
        ...
        randomPos_X = 0f;
        restOn = false;
        boom_PeriodTime = GameDatas.Inst.Get_SelectSeting.boomTimeSection;
        boomNum = GameDatas.Inst.Get_SelectSeting.creatBoomCount;
    }
    ...
    private void Spawn()
    {
        ...
        fruit_PeriodTime = Random.Range(GameDatas.Inst.Get_SelectSeting.fruitTimeSection_Min, GameDatas.Inst.Get_SelectSeting.fruitTimeSection_Max);
        
        if (fruit_spawnTime >= fruit_PeriodTime)
            FruitSpwn(GameDatas.Inst.Get_SelectSeting.creatFruit_MinCount[listIdx], GameDatas.Inst.Get_SelectSeting.creatFruit_MaxCount[listIdx]);
        if (boom_spawnTime >= boom_PeriodTime)
            BoomSpwn();
        if (stop_PeriodTime >= stop_SpawnTime)
            StopTime();

        for (int i = 0; i < GameDatas.Inst.Get_SelectSeting.boomAddCreatCount.Count; i++)
        {
            if (GameManager.Inst.curTime == GameDatas.Inst.Get_SelectSeting.boomAddCreatCount[i])
            {
                BoomSpwn();
            }
        }
